...
- [x] 2.3 `src/events/event-handler.ts`：新增 `scanStalledSessions()`——超时判定（`stall_timeout_ms`）+ 重复提醒节流（`stall_interval_ms`），调用 `mapStallEvent` 发送，失败仅记日志；不写入 `erroredSessions`
- [x] 2.4 `src/index.ts`：初始化 `setInterval` 调用 `scanStalledSessions()`（间隔 `stall_check_interval_ms`），定时器句柄随进程生命周期
